...
```{r include = FALSE}
    info_df = rownames_to_column(info_df, var = 'Model Name')
    colnames(info_df) = c('Model Name', 'Model', 'Linear fold-change', 'FDR')
    info_kable = kable(info_df, row.names = TRUE, format.args = list(big.mark = ',', scientific = FALSE)) %>%
...
```{r echo = FALSE, results = 'asis', message = FALSE, eval=F} # TWS FIXME
    ...
    for(model_name in names(comparisons)) {
        ...
        for(comparison in comps_list){
          ...
          volcano_file = normalizePath(sprintf(diffex_volcano_file, DIFFEX_DIR, model_name_for_files, comparison))
          cat(sprintf('![**Figure:** Volcano plot for %s.](%s) \n\n', comparison, volcano_file))
          annot_file = normalizePath(sprintf(diffex_annot_file, DIFFEX_DIR, model_name_for_files, comparison))
