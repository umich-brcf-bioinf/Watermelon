...
countData <- read.table(file="HTSeq_counts.txt", header = TRUE, sep = "\t", row.names = 1, strip.white = TRUE, stringsAsFactors = FALSE)
countData <- round(countData)

#read in metaDataFile
colData <- read.table(file = "singer_metadata.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE)

#read in contrastFile
contrastData <- read.table(file = "singer_contrasts.txt", header=TRUE, sep = "\t", stringsAsFactors = FALSE, strip.white = TRUE)
...
for (i in 1:nrow(contrastData)){
  ...
  diffexDataDEG <- subset(x = diffexData, subset = abs(diffexData$log2FoldChange) >= log2(1.5) & diffexData$padj <= 0.05)
