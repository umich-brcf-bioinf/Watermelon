...
#gp <- ggplotly(gp)
#path_name <- file.path(getwd(),"plots","PCAplot_All.html")
#htmlwidgets::saveWidget(gp, file = path_name)
#
#z <- 1
#for (i in names(sampleColData[1:length(sampleColData)])) {
#  p <- plotPCA(rld, intgroup = i) #get PCA components
...
#  g <- ggplot(p$data, aes(x = PC1, y = PC2, color = Replicates, shape = Group)) + scale_shape_manual(values=1:nlevels(Group)) + geom_point(size=2) + ggtitle(label = as.character(i)) + theme(plot.title = element_text(hjust = 0.5)) + theme_classic()
#  g <- ggplotly(g)
#  path_name <- file.path(paste0(getwd(),"/plots/PCAplot_",as.character(i),".html"))
...
#bp <- ggplotly(bp) 
#path_name <- file.path(getwd(),"plots","BoxPlot_RawCounts.html")
...
#bpn <- ggplotly(bpn) 
#path_name <- file.path(getwd(),"plots","BoxPlot_DepthNormalizedCounts.html")
...
#bpr <- ggplotly(bpr) 
#path_name <- file.path(getwd(),"plots","BoxPlot_RlogNormalizedCounts.html")
...
#  theme(legend.position = "right") + ylab('Density') + xlab(expression(paste(Log[2]," counts"))) + ggtitle('Non-normalized Counts') + theme_classic()
#dp <- ggplotly(dp) 
#path_name <- file.path(getwd(),"plots","DensityPlot_RawCounts.html")
...
#  theme(legend.position = "right") + ylab('Density') + xlab(expression(paste(Log[2]," depth-normalized counts"))) + ggtitle('Depth-normalized Counts') + theme_classic()
#dpn <- ggplotly(dpn) 
#path_name <- file.path(getwd(),"plots","DensityPlot_DepthNormalizedCounts.html")
...
#  theme(legend.position = "right") + ylab('Density') + xlab(expression(paste(Regularized-Log[2]," normalized counts"))) + ggtitle('Rlog-normalized Counts') + theme_classic()
#dpr <- ggplotly(dpr) 
#path_name <- file.path(getwd(),"plots","DensityPlot_RlogNormalizedCounts.html")
...
#  gcp <- ggpairs(data = gc[1:ncol(gc)], upper = list(continuous = wrap(ggally_cor, use = "pairwise.complete.obs", method = "spearman")), title = as.character(uniqGroups[i])) + theme_bw()
#  gcp <- ggplotly(gcp)
#  path_name <- file.path(paste0(getwd(),"/plots/CorrelMatrix_",uniqGroups[i],".html"))
...
#acp <- ggplotly(acp)
#path_name <- file.path(getwd(),"plots","CorrelMatrix_All.html")
...
for (i in 1:nrow(contrastData)){
  ...
  #MA plot
  pdf(file = paste0('plots/MAplot_',contrastData$factor[i],".",contrastData$base_file_name[i],'.pdf'), onefile = FALSE)
...
#  p <- p + ggtitle(as.character(contrastData$base_file_name[i]))
#  mp <- ggplotly(p)
#  path_name <- file.path(paste0(getwd(),"/plots/MAplot_",contrastData$factor[i],'.',contrastData$base_file_name[i],".html"))
#  htmlwidgets::saveWidget(mp, file = path_name)
  
  #Volcano plot
  pdf(file = paste0('plots/Volcano_',contrastData$factor[i],'.',contrastData$base_file_name[i],'.pdf'), onefile = FALSE)
...
#  p <- p + ggtitle(as.character(contrastData$base_file_name[i]))
#  vp <- ggplotly(p)
#  path_name <- file.path(paste0(getwd(),"/plots/Volcano_",contrastData$factor[i],'.',contrastData$base_file_name[i],".html"))
