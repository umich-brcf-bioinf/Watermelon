...
class DiffexExcelTest(unittest.TestCase):
    ...
    def test_commandReturnsCorrectRowAndColumnCount(self):
        with TempDirectory() as temp_dir:
            ...
            input_file_contents = \
'''#Select Differentially Expressed Transcripts Based on Three Criteria
#Test Status OK
#FDR (q-value) <= 0.05
#Fold Change >= Threshold or <= 1/Threshold, where Threshold = 1.5
#
...
Eif2s3y|Eif2s3y|26908|desc3|0.154875|30.2905|7.6|5.1|5.00E-05'''.replace('|', '\t')
            ...
            command = 'python {} -g {} -i {} {} {}'.format(script_name,
                                        input_filename,
                                        input_filename,
                                        output_filename,
                                        redirect_output)
