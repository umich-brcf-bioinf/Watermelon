...
class _CommandValidator(object):
    ...
    def _validate_genomebuild(args):
        x_genome_references = yaml.load(args.x_genome_references)
        if args.genome_build not in x_genome_references:
            msg = f"genome {args.genome_build} is not found"
...
def _validate_sample_sheet(samples, sheet_file):
    ...
    try:
        samplesheet = pd.read_csv(sheet_file, comment="#").set_index(
            args.x_sample_column, drop=True
        )
    except KeyError:
        msg = "The sample sheet must have a column labeled '{}'".format(
            args.x_sample_column
        )
        raise _InputValidationError(msg)
    ...
    if not sheet_samples.issubset(input_samples):
        msg = (
            "The following sample names cannot be found in the input directories: "
...
def main(sys_argv):
    ...
    try:
        ...
        with open(args.x_template_config, "r") as template_config_file:
            ...
            )  # Use ruamel_yaml to preserve comments
        with open(args.x_genome_references, "r") as genome_references_file:
            genome_references = yaml.load(genome_references_file, yaml.FullLoader)[
                args.genome_build
            ]
        _validate_sample_sheet(input_summary.samples, args.sample_sheet)
        config_dict = _make_config_dict(template_config, genome_references, args)
