...
Clustering is a method to visualize the similarity and difference among samples. Unsupervised clustering of the Pearson correlation coefficients on the gene expression profiles of pairwise samples is shown below. [REPLACE: Anything of note? Mention it here.]

```{r echo = FALSE, results = 'asis', message = FALSE, out.width = "100%", out.height = "100%", fig.show = 'hold'}
for(factor_cov in snakemake@params[['phenotypes']]) {
  cat(sprintf('\n\n #### %s \n\n', factor_cov))
  file = sprintf(qc_heatmap_file, diffex_dir, factor_cov)
  cat(sprintf('![**Figure:** Unsupervised clustering of %s beta values colored by %s.](%s) \n\n', value, factor_cov, file))
}
```

### PCA

Principal component analysis (PCA) is another method to visualize the similarity and difference among samples. PCA was applied to the gene expression profiles of the samples. [REPLACE: Anything of note? Mention it here.]
...
for(factor_cov in snakemake@params[['phenotypes']]) {
  ...
  for(value in c('top100', 'top500')) {
    ...
    cat(sprintf('![**Figure:** PCA plot (PC1 vs PC2) of %s M-values colored by %s.](%s) \n\n', value, factor_cov, pca_12_png))
  }
}
```